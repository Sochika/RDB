...
@section('content')
 ...
 <div class="card">
  <div class="text-center mb-6">
    <h4 class="address-title mb-2">Edit Beat {{$beat->name}}</h4>
    <p class="address-subtitle">Edit beat Details</p>
  </div>
  <form id="addNewAddressForm" class="row g-6" action="{{ route('beat.update', ['id' => $beat->id]) }}" method="POST">
    @csrf
        ...
        <div class="col-md mb-md-0 mb-4">
          <div class="form-check custom-option custom-option-icon">
            <label class="form-check-label custom-option-content" for="customRadioHome">
              ...
              <input name="type" class="form-check-input" type="radio" value="resident" id="customRadioHome" {{$beat->type == 'resident' ? 'checked' : ''}} />
            </label>
          </div>
        </div>
        <div class="col-md mb-md-0 mb-4">
          <div class="form-check custom-option custom-option-icon">
            <label class="form-check-label custom-option-content" for="customRadioOffice">
              ...
              <input name="type" class="form-check-input" type="radio" value="office" id="customRadioOffice" {{$beat->type == 'office' ? 'checked' : ''}}  />
            </label>
          </div>
        </div>
      </div>
    ...
    <div class="col-12 col-md-6">
      ...
      <input type="text" id="modalAddressFirstName" name="first_name" class="form-control" required value="{{$beat->beatBranches[0]->first_name ?? ''}}"/>
    </div>
    <div class="col-12 col-md-6">
      <label class="form-label" for="modalAddressLastName">Contact Last Name</label>
      <input type="text" id="modalAddressLastName" name="last_name" class="form-control" required value="{{$beat->beatBranches[0]->last_name ?? ''}}"/>
    </div>

    <div class="col-12 col-md-6">
      <label class="form-label" for="phone_number">Contact Phone Number</label>
      <input type="text" id="phone_number" name="phone_number" class="form-control" required value="{{$beat->phone_number}}" />
    </div>
    <div class="col-12 col-md-6">
      <label class="form-label" for="email">Contact Email</label>
      <input type="email" id="email" name="email" class="form-control" value="{{$beat->email}}" />
    </div>

    <div class="col-12 ">
      <label class="form-label" for="modalAddressAddress1">Address</label>
      <input type="text" id="modalAddressAddress1" name="Address1" class="form-control" required value="{{$beat->beatBranches[0]->address ?? ''}}" />
    </div>
    {{-- <div class="col-12">
    ...
    <div class="col-12 col-md-6">
      ...
      <input type="text" id="modalAddressArea" name="area" class="form-control" value="{{$beat->beatBranches[0]->area ?? ''}}" required />
    </div>
    <div class="col-12 col-md-6">
      <label class="form-label" for="modalAddressLandmark">Landmark</label>
      <input type="text" id="modalAddressLandmark" name="landmark" class="form-control" value="{{$beat->beatBranches[0]->landmark ?? ''}}" />
    </div>
    <div class="col-12 col-md-6">
      <label class="form-label" for="modalAddressCity">City</label>
      <input type="text" id="modalAddressCity" name="city" class="form-control" value="{{$beat->beatBranches[0]->city ?? ''}}" />
    </div>
    <div class="col-md-6 mb-6">
      ...
      <select id="selectpickerLiveSearch" class="form-select selectpicker w-100" name="state" data-style="btn-default" data-live-search="true">
        ...
        <option {{$beat->beatBranches[0]->state == $state->name ? 'selected' : ''}} data-tokens="{{ $state->name }}">{{ $state->name }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div class="col-12 col-md-6">
      ...
      <input type="date" id="onboard_date" name="onboard_date" class="form-control" value="{{$beat->onboard_date}}" />
    </div>
